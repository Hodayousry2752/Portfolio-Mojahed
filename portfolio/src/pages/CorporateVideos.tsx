...
import { corporateVideos } from "../data/corporateVideos";
...
const controlsTimeoutRef = useRef<number | null>(null);
  ...
  const handleShare = (video: any, e: React.MouseEvent) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: video.title,
        text: isRTL ? 'شاهد هذا الفيديو الرائع' : 'Check out this amazing video',
        url: window.location.origin + '/corporate',
      }).catch(console.error);
    } else {
      navigator.clipboard.writeText(window.location.origin + '/corporate')
        .then(() => {
          alert(isRTL ? 'تم نسخ الرابط إلى الحافظة' : 'Link copied to clipboard');
        })
        .catch(() => {
          const textArea = document.createElement('textarea');
          textArea.value = window.location.origin + '/corporate';
    ...
    <div className={`min-h-screen bg-black text-white ${isRTL ? "rtl" : "ltr"}`} dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="font-bold text-lg">{t("projects.categories.corporate.title", "Corporate Coverage & Events")}</h1>
                  <p className="text-gray-400 text-xs">{corporateVideos.length} {isRTL ? "فيديو" : "videos"}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        >
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">
              {isRTL ? "مكتبة فيديوهات التغطية والمناسبات" : "Corporate Coverage & Events Library"}
            </h1>
            <p className="text-gray-400 max-w-2xl mx-auto">
              {isRTL ? "تصفح مجموعة فيديوهات التغطية والمناسبات الاحترافية" : "Browse our collection of professional corporate coverage and event videos"}
            </p>
          </div>
            ...
            <div className="flex-1 w-full">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 pl-11 bg-zinc-900/50 border border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500/50 focus:border-transparent transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center bg-zinc-900/50 rounded-xl p-1">
                <button
                  ...
                  className={`px-3 py-2 rounded-lg transition-all ${viewMode === "grid" ? 'bg-yellow-500/20 text-yellow-400' : 'hover:bg-zinc-800/50'}`}
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg transition-all ${viewMode === "list" ? 'bg-yellow-500/20 text-yellow-400' : 'hover:bg-zinc-800/50'}`}
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 rounded-xl bg-zinc-900/50 border border-zinc-800 hover:border-yellow-500/30 transition-all"
                >
        ...
        {filteredVideos.length === 0 ? (
          ...
          >
            <div className="w-32 h-32 mx-auto mb-6 rounded-full bg-gradient-to-r from-yellow-500/10 to-amber-500/10 flex items-center justify-center">
            ...
            {searchQuery && (
              <motion.button
                ...
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-yellow-600 to-amber-600 font-medium"
              >
          ...
          >
            {filteredVideos.map((video) => (
              ...
              >
                <div className="relative overflow-hidden rounded-2xl border border-gray-800/40 bg-gray-900/30 transition-all duration-300 group-hover:border-yellow-500/50 group-hover:bg-gray-900/50">
                  {/* Video Thumbnail */}
                  <div className="relative aspect-video overflow-hidden rounded-t-2xl">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <motion.div
                        ...
                        className="relative"
                      >
                        <div className="absolute inset-0 bg-gradient-to-r from-yellow-600 to-amber-600 rounded-full blur-xl opacity-30 group-hover:opacity-50 transition-opacity" />
                    ...
                    {video.featured && (
                      <div className="absolute top-4 right-4">
                        <div className="px-3 py-1.5 bg-gradient-to-r from-yellow-600 to-amber-600 backdrop-blur-sm rounded-lg text-sm font-medium">
                          {isRTL ? "مميز" : "Featured"}
                        </div>
                      </div>
                    )}
          ...
          >
            {filteredVideos.map((video) => (
              <motion.div
                ...
                onClick={() => handlePlayVideo(video)}
              >
                <div className="flex items-center gap-4 p-4 rounded-2xl border border-gray-800/30 bg-gray-900/30 transition-all duration-300 group-hover:border-yellow-500/50 group-hover:bg-gray-900/50">
                  {/* Thumbnail */}
                  <div className="relative w-40 aspect-video rounded-xl overflow-hidden flex-shrink-0">
                    ...
                    {video.featured && (
                      <div className="absolute top-2 right-2 px-2 py-1 bg-gradient-to-r from-yellow-600 to-amber-600 rounded text-xs">
                        {isRTL ? "مميز" : "Featured"}
                      </div>
                    )}
                  </div>
                ...
                <div className="absolute inset-0 z-20 bg-black/80 flex items-center justify-center">
                  <div className="text-center">
                    <motion.div
                      ...
                      className="w-16 h-16 mx-auto mb-4 rounded-full border-4 border-yellow-500/30 border-t-yellow-500"
                    >
                      <Loader2 className="w-full h-full text-yellow-500 p-4" />
                    </motion.div>
                ...
                {(!isPlaying || showControls) && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <motion.button
                      ...
                      className="relative"
                    >
                      <div className="absolute inset-0 bg-gradient-to-r from-yellow-600 to-amber-600 rounded-full blur-2xl opacity-50" />
                  ...
                  >
                    ...
                    <div className="mb-4">
                      <input
                        ...
                        value={currentTime}
                        onChange={handleSeek}
                        className="w-full h-2 bg-gray-700/50 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gradient-to-r [&::-webkit-slider-thumb]:from-yellow-600 [&::-webkit-slider-thumb]:to-amber-600"
                      />
      ...
      <footer className="mt-16 pt-12 pb-8 border-t border-gray-800/30">
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 mb-12">
            <div>
              <h3 className="text-2xl font-bold text-white mb-3">
                {isRTL ? "مناسبة أو حدث قادم؟" : "Upcoming event or occasion?"}
              </h3>
              <p className="text-gray-400 max-w-lg">
                {isRTL ? "دعنا نعمل معاً لتغطية مناسبتك أو حدثك باحترافية عالية" : "Let's work together to professionally cover your event or occasion"}
              </p>
            </div>
            <div className="flex items-center gap-4">
      <MotionLink
        ...
        whileTap={{ scale: 0.95 }}
        className="px-8 py-3 bg-gradient-to-r from-yellow-600 to-amber-600 text-white rounded-xl hover:shadow-lg hover:shadow-yellow-500/25 transition-all font-bold flex items-center gap-3"
      >
